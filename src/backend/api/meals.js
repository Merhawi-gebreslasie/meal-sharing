...
if(limit!==Number.MAX_VALUE) {
  ...
  if (availableReservations==true) {
    meals
      ...
      .groupBy("meal_id").having(Knex.raw('sum(max_reservation-SUM(number_of_guests>0'))


// knex.raw('meal.max_reservations-SUM(reservations.number_of_guests) as available_reservations'))
// .groupBy('meal_id').having('meal.max_reservations','>',knex.raw('SUM(reservations.number_of_guests)' ))
//   //  meals=   result.filter((meal) => {
...
try {
  const mealToAdd={ title: req.body.title, description: req.body.description, location: req.body.location, when: req.body.when, max_reservations: req.body.max_reservations, price: req.body.price, created_date: req.body.created_date }
...
if (isNaN(parseInt(req.params.id))) {
  ...
  msq:'ID must be a number'
  });
}
else {
  try {
    const mealById = await knex("meal").where({'id': parseInt(req.params.id)});
    res.send(mealById);
  } catch (error) {
...
msg:'ID  must be a number'});
  ...
  try {
    ...
    const updateMeal = await knex("meal").where({ id: parseInt(req.params.id) }).update({ description: req.body.description });
    res.json(updateMeal);
  } catch (error) {
    throw error;
  }
...
if (isNaN(parseInt(req.params.id))) {
  res.json(404).json({status:'bad request',
msg:'ID to delete is not a number'});
}
else {
  try {
    const mealToDelete = await knex("meal").where({ id: parseInt(req.params.id) }).del();
